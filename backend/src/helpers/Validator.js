...
class Validator {
     static validateNumber(number, range = false, text = "number") {
          ...
          if (
               ...
               throw { status: "error", code: 400, message: `${text} invalid` }
          }
     }
     ...
     static validateString(string, size = false, text = "string") {
          ...
          if (size || size.length == 2) {
               if (string.length > size[1] || string.length < size[0])
                    throw {
                         status: "error",
                         code: 400,
                         message: `${text} with invalid size`,
                    }
          }
     ...
     static validateDatetime(datetime, text = "datetime") {
          ...
          if (!REGEX_DT.test(datetime))
               throw {
                    message: `${text} invalid, must be in format: YYYY-MM-DD hh:mm:ss`,
                    status: "error",
                    code: 400,
               }
     }
     ...
     static validateDate(data, text = "data") {
          ...
          if (!REGEX_DATA.test(data))
               throw {
                    message: `${text} invalid, must be in format: YYYY-MM-DD`,
                    status: "error",
                    code: 400,
               }
     }
