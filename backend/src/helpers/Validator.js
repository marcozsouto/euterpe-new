...
class Validator{
    ...
    static validateString(string, size = false, text = "string") {
        if(!string) throw { status: -1, code: 400, mensagem: `${text} invalid` }
        
        if(size || size.length == 2){
            if(string.length > size[1] || string.length < size[0] ) throw { status: -1, code: 400, mensagem: `${text} with invalid size` };
        }
    ...
    static validateDatetime(datetime, text = 'datetime') {
        if (typeof datetime != 'string') throw { mensagem: `${text} must be string`, status: -1, code: 400 };
        if (!REGEX_DT.test(datetime)) throw { mensagem: `${text} invalid, must be in format: YYYY-MM-DD hh:mm:ss`, status: -1, code: 400 };
    }
      
    static validateDate(data, text = 'data') {
    if (typeof data != 'string') throw { mensagem: `${text} must be string`, status: -1, code: 400 };
    if (!REGEX_DATA.test(data)) throw { mensagem: `${text} invalid, must be in format: YYYY-MM-DD`, status: -1, code: 400 };
    }
}
